...
export const ThemeContextProvider = ({ children }) => { // 注意這是元件，包裝上下文的提供以及狀態，ThemeProvider會撞名
  ...
  useEffect(() => { // 在關閉瀏覽器保留theme，以便重啟網頁及路由更新時取得theme
    const checkTheme = () => {
      const theme = localStorage.getItem('theme')
  ...
  return <ThemeContext.Provider  //  將上下文物件提供為React component 使子元件可以共享上下文
    value={{
      theme,
      toggleTheme
    }}>
    <ThemeProvider // 這個Provider 給包覆層CSS以及styledCSS提供theme變數 注意這是套件元件
      theme={theme === "light" ? lightTheme : darkTheme}> {/* 將你寫的theme內容丟進去 */}
      {children}
    </ThemeProvider >
